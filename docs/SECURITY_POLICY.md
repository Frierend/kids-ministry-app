...
- A secure random salt is generated with `expo-crypto` and stored through `expo-secure-store`.
- Failed PIN attempts are tracked in SecureStore for the current attempt window.
- A recoverable temporary cooldown is applied after repeated failed attempts.
...
- The PIN lockout is temporary and recoverable; it is not a substitute for full device security.
